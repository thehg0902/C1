...
1. Read agency/state/BUILD_STATE.md. If "Client:" is unset, this is a fresh build:
   set client name from agency/client/client.md, set Started date.
2. Find the first phase not marked done and execute phases in order:
   - Phase 0 intake: run `python3 agency/scripts/validate-client-md.py`. If it
     reports blockers, write them as questions to agency/state/QUESTIONS.md, mark
   ...
   - Phase 1 architecture: use the site-architecture skill. Output: page
     map + section list per page, appended to agency/state/DECISIONS.md.
   ...
   - Phase 3 content: use the copywriting skill. Output: final copy for
     ...
     (create page skeletons per agency/contracts/file-structure.md if absent).
   - Phase 4 media: use the media-generation skill. Produce a storyboard in
     agency/state/MEDIA_LOG.md (status=planned). STOP. The human must set
