...
2. Manual review after the script, in this order:
   ...
   d. Resize pass: 360px, 768px, 1280px - no overflow, no overlap,
      tap targets >= 44px.
   e. Disable JS mentally: does every section still make sense
      (component-api rule 5)?
3. Any MEDIA_LOG row with status=generated but file missing from assets/,
   or asset in use without a log row: FAIL.
