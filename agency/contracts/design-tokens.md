# Contract: Design Tokens  (v1.0.0)

All visual values are CSS custom properties defined ONCE in
`src/css/tokens.css` on `:root`. No component may hardcode a color, font
size, spacing value, radius, shadow, or duration.
...
## Rules
1. Producer: the design-tokens skill writes `src/css/tokens.css`.
